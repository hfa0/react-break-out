...
class BreakOut extends Component {
  ...
  resetGame() {
    this.stopGame();
    this.defaultState.nodes = createNodes(this.screenWidth,this.screenHeight,this.line)
    this.setState({...this.defaultState, isGameOver : false});
  }
  ...
  moveBar(e) {
    ...
    if ((xBar+dxBar) < 0 || (xBar+parseFloat(this.barWidth)+dxBar)>this.screenWidth) return ;
  ...
  bounceSucceed(xBall, xBar) {
    var a = (xBall >= xBar && xBall <= xBar+this.barWidth)
    return a
  }
